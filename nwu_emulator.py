...
class nwu_swu():
    ...
    def __init__(self, source_address,epdg_address,apn,modem,default_gateway,mcc,mnc,imsi,ki,op,opc,interface_type,imei,free5gc,free5gc_userplane_ip_address):
...
#######################################################################################################################
    ...
    def decode_payload_type_sa(self, data):
        ...
        if data[5]!= 0:
            spi = data[8:8+data[6]]            
        return [data[4],data[5],spi,data] # proposal number, protocol_id, spi, data (data may be useful when answering back with the same proposal)
...
#######################################################################################################################
    ...
    def encode_5g_plmnid(self,mccmnc):
        return (AN_PARAMETER_TYPE_SELECTED_PLMN_ID, b'\x00\x03' + return_plmn(mccmnc))  

    def encode_5g_establishment_cause(self,value):
        return (AN_PARAMETER_TYPE_ESTABLISHMENT_CAUSE, b'\x00' + bytes([value]))

    def encode_5g_guami(self,mccmnc, amf_region, amf_set_id, amf_pointer):
        return (AN_PARAMETER_TYPE_GUAMI, b'\x00' + return_plmn(mccmnc) + bytes([amf_region]) + struct.pack("!H", (amf_set_id<<6) + amf_pointer))
    ...
    def encode_5g_nssai(self,nssai_list): # [(sst1, sd1), (sst2,sd2), etc...] Needs adaptation to support sst only, and also mapped_hplmn_sst and mapped_hplmn_ssd
        ...
        for i in nssai_list:
            if self.free5gc == True:
                snssai += b'\x00' + bytes([4]) + bytes([i[0]]) + struct.pack("!I",i[1])[1:]    # first byte i think is not correct. Added to use free5GC N3IWF
            else:
                snssai += bytes([4]) + bytes([i[0]]) + struct.pack("!I",i[1])[1:]
                
        return (AN_PARAMETER_TYPE_REQUESTED_NSSAI, b'\x00' + bytes([len(snssai)]) + snssai)
    ...
    def encode_and_encrypt_nas(self,plain_nas,security_header):
        ...
        if self.free5gc == True: # free5GC uses BEARER_ID_NAS_CONNECTION_IDENTIFIER_3GPP instead of BEARER_ID_NAS_CONNECTION_IDENTIFIER_NON_3GPP as stated in 33.501 section 7.2.1
            nas_pdu_response_encrypted = nas_encrypt_func(plain_nas, self.COUNT_UP, DIRECTION_UP, self.KEY_EA[self.nas_enc_alg], self.nas_enc_alg, BEARER_ID_NAS_CONNECTION_IDENTIFIER_3GPP)                                    
            nas_hash = nas_hash_func(nas_pdu_response_encrypted, self.COUNT_UP, DIRECTION_UP, self.KEY_IA[self.nas_int_alg], self.nas_int_alg, BEARER_ID_NAS_CONNECTION_IDENTIFIER_3GPP) 
        else: #according to 33.501:
...
### USER PLANE FUNCTIONS AND INTER PROCESS COMMUNICATION ####
    ...
    def set_routes(self):
        ...
        if self.ip_address_list != []:
            subprocess.call("ip addr add " + self.ip_address_list[0] + "/32 dev tun" + str(TUNNEL_ID_SWU), shell=True)
            #set route
        ...
        if self.ipv6_address_list != []:
            ...
            subprocess.call("ip -6 addr add " + ipv6_address_identifier + "/64 dev tun" + str(TUNNEL_ID_SWU), shell=True)
            subprocess.call("route -A inet6 add ::/1 dev tun" + str(TUNNEL_ID_SWU), shell=True) 
            subprocess.call("route -A inet6 add 8000::/1 dev tun" + str(TUNNEL_ID_SWU), shell=True)
    ...
    def set_routes_nwu_tcp(self):
        ...
        if self.tunnel_ipv4_address is not None:
            subprocess.call("ip addr add " + self.tunnel_ipv4_address + "/32 dev tun" + str(TUNNEL_ID_NWU_TCP), shell=True)
            subprocess.call("route add " + self.nas_ip_address + "/32 gw " + self.tunnel_ipv4_address, shell=True)
    ...
    def set_routes_nwu_userplane(self):
        ...
        subprocess.call("ip addr add " + userplane_ip_address + "/32 dev tun" + str(TUNNEL_ID_NWU_USERPLANE_DATA), shell=True)
        subprocess.call("route add -net 0.0.0.0/1 gw " + userplane_ip_address , shell=True)    
        subprocess.call("route add -net 128.0.0.0/1 gw " + userplane_ip_address, shell=True)      
                    ...
                    if sock == socket_tcp:       
                        ...
                        if decode_list[0] == INTER_PROCESS_TCP:
                            if decode_list[1][0][0] == INTER_PROCESS_IE_TCP_MESSAGE:
                                ...
                                if len(packet)>0: socket_tcp.send(packet)   
            ...
            for sock in read_sockets:    
                if sock == self.tunnel:
                    ...
                    if encr_alg is not None:
                        if self.free5gc == False and self.interface_type == NWU: # 24.502 section 9.4
                            tap_packet = struct.pack("!H",len(tap_packet)) + tap_packet
                    ...
                    if encr_alg_userplane is not None:
                        ip_header = get_ip_header_with_checksum(tunnel_ipv4_address, up_ipv4_address, 0x2F, len(tap_packet)+4)
                        #GRE Header:
                        if self.free5gc == True:
                            if tap_packet[0]//16 == 4: #ipv4
                                gre_header = fromHex('00000800')
                            else: #ipv6
                                gre_header = fromHex('000086dd')
                        else: # 24.502, section 9.3.3:
                            gre_header = fromHex('20000000') + bytes([qfi_userplane]) + fromHex('000000') #no RQI in uplink packets
                        ...
                        if packet[0:4] == b'\x00\x00\x00\x00': #is ike message
                            ...
                            if encr_alg is not None:
                                ...
                                if decrypted_packet is not None:
                                    if self.free5gc == True or self.interface_type == SWU:
                                        os.write(self.tunnel,decrypted_packet)
                                    else: #24.502 9.4 (NAS message envelope)
                                        os.write(self.tunnel,decrypted_packet[2:])
                            ...
                            if encr_alg_userplane is not None:
                                ...
                                if decrypted_packet is not None:
                                    #Removing GRE header. Assumes IP header with 20 bytes.
                                    if self.free5gc == True:                                    
...
#### MESSAGES ####
    ...
    def answer_CREATE_CHILD_SA_CHILD_nwu(self,lowest = 0, proposal_bytes = None): #DONE to work with free5GC
...
################################## STATES FOR NWU ##################################
                        ...
                        if i[1][4][2] in (EAP_5G_NAS,) and i[1][4][4] is not None:                          
                            ...
                            if get_nas_ie_by_name(nas_decoded,IE_SECURITY_HEADER) in (INTEGRITY_PROTECTED,INTEGRITY_PROTECTED_WITH_NEW_5GS_MAS_SECURITY_CONTEXT,):
                                ...
                                if get_nas_ie_by_name(nas_decoded,IE_MESSAGE_TYPE) == SECURITY_MODE_COMMAND:
                                    ...
                                    mac_calculated = nas_hash_func(nas_decoded_encrypted, sqn, DIRECTION_DOWN, self.KEY_IA[self.nas_int_alg], self.nas_int_alg, BEARER_ID_NAS_CONNECTION_IDENTIFIER_3GPP) 
    ...
    def state_7_nwu(self, packet):
        ...
        if get_nas_ie_by_name(nas_decoded,IE_SECURITY_HEADER) in (INTEGRITY_PROTECTED_AND_CIPHERED,):
            ...
            nas_pdu_decrypted = nas_encrypt_func(nas_decoded_encrypted, self.COUNT_DOWN, DIRECTION_DOWN, self.KEY_EA[self.nas_enc_alg], self.nas_enc_alg, BEARER_ID_NAS_CONNECTION_IDENTIFIER_3GPP)                                    
    ...
    def state_n3iwf_create_sa_child_nwu(self): #NOT DONE
        ...
        if isESP == True:
            ...
            packet = self.answer_CREATE_CHILD_SA_CHILD_nwu(proposal_bytes=received_proposal_bytes)
            
            #packet = self.answer_CREATE_CHILD_SA_CHILD_nwu()
            #self.spi_resp_child = self.sa_spi_list[0] #only one proposal was made
            ...
            if self.free5gc == True and self.userplane_ip_address is None: 
                #create routes for userplane ip address given by CLI
                self.set_routes_nwu_userplane()
...
#        [INTEG,AUTH_HMAC_SHA1_96],
    ...
    a = nwu_swu(options.source_addr,destination_addr,options.apn,options.modem,
        ...
        options.free5gc,options.free5gc_userplane_ip_address)
